...
void reporter_thread_entry(void *p1, void *p2, void *p3) {
    ...
    while (1) {
        ...
        // Map enum to string
        const char *mode_str = (status.mode == MODE_AUTO) ? "auto" : (status.mode == MODE_NIGHT) ? "night" : "manual";
        const char *speed_str = (status.fan_speed_enum == 0)
                                    ? "off"
                                    : (status.fan_speed_enum == 1)
                                          ? "low"
                                          : (status.fan_speed_enum == 2)
                                                ? "medium"
                                                : "high";
